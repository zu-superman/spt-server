...
from "@spt/context/ApplicationContext";
import {
  ContextVariableType
}
from "@spt/context/ContextVariableType";
import {
  BotGenerator
}
from "@spt/generators/BotGenerator";
import {
  BotDifficultyHelper
}
from "@spt/helpers/BotDifficultyHelper";
import {
  BotHelper
}
from "@spt/helpers/BotHelper";
import {
  ProfileHelper
}
from "@spt/helpers/ProfileHelper";
import {
  WeightedRandomHelper
}
from "@spt/helpers/WeightedRandomHelper";
import {
  MinMax
}
from "@spt/models/common/MinMax";
import {
  ICondition,
  IGenerateBotsRequestData
}
from "@spt/models/eft/bot/IGenerateBotsRequestData";
import {
  IPmcData
}
from "@spt/models/eft/common/IPmcData";
import {
  IBotBase
}
...
from "@spt/models/eft/common/tables/IBotCore";
import {
...
from "@spt/models/eft/match/IGetRaidConfigurationRequestData";
import {
  ConfigTypes
}
...
from "@spt/models/enums/SideType";
import {
  WildSpawnTypeNumber
}
from "@spt/models/enums/WildSpawnTypeNumber";
import {
  IBotGenerationDetails
}
from "@spt/models/spt/bots/BotGenerationDetails";
import {
  IBotConfig
}
...
from "@spt/models/spt/utils/ILogger";
import {
  ConfigServer
}
...
from "@spt/services/BotGenerationCacheService";
import {
  DatabaseService
}
from "@spt/services/DatabaseService";
import {
  LocalisationService
}
...
from "@spt/services/MatchBotDetailsCacheService";
import {
  SeasonalEventService
}
from "@spt/services/SeasonalEventService";
import {
  ProgressWriter
}
from "@spt/utils/ProgressWriter";
import {
  RandomUtil
}
from "@spt/utils/RandomUtil";
import type {
  ICloner
}
from "@spt/utils/cloners/ICloner";
import {
  inject,
  injectable
}
...
@injectable() export class BotController {
  ...
  /**
     ...
     */
  public getBotDifficulty(type: string, diffLevel: string, raidConfig ? :IGetRaidConfigurationRequestData, ignoreRaidSettings = false, ) : IDifficultyCategories {
  ...
  public getAllBotDifficulties() : Record < string,
  any > {
    const result = {};
    ...
    for (let botType of botTypes) {
      ...
      botType = this.botHelper.isBotPmc(botType) ? this.botHelper.getPmcSideByRole(botType).toLowerCase() : botType.toLowerCase();

      const botDetails = botTypesDb[botType];
      if (!botDetails ? .difficulty) {
        this.logger.warning(`Unable to find bot: $ {
          botType
        }
        difficulty values`);
  ...
  /**
     ...
     * @returns IBotBase[]
     */
  protected async generateAndCacheBots(request: IGenerateBotsRequestData, pmcProfile: IPmcData | undefined, sessionId: string, ) : Promise < void > {
    ...
    const conditionPromises = request.conditions.map(async(condition) = >{
      ...
      const botGenerationDetails = this.getBotGenerationDetailsForWave(condition, pmcProfile, allPmcsHaveSameNameAsPlayer, raidSettings,
      // Spawn the higher of the preset cache amount, or the requested amount
      Math.max(this.getBotPresetGenerationLimit(condition.Role), condition.Limit), this.botHelper.isBotPmc(condition.Role), );
  ...
  protected getBotGenerationDetailsForWave(condition: ICondition, pmcProfile: IPmcData | undefined, allPmcsHaveSameNameAsPlayer: boolean, raidSettings: IGetRaidConfigurationRequestData, botCountToGenerate: number, generateAsPmc: boolean, ) : IBotGenerationDetails {
    return {
      ...
      locationSpecificPmcLevelOverride: this.getPmcLevelRangeForMap(raidSettings ? .location),
      // Min/max levels for PMCs to generate within
      isPlayerScav: false,
  ...
  /**
     ...
     */
  protected async generateWithBotDetails(condition: ICondition, botGenerationDetails: IBotGenerationDetails, sessionId: string, ) : Promise < void > {
    ...
    const cacheKey = this.botGenerationCacheService.createCacheKey(botGenerationDetails.eventRole ? ?botGenerationDetails.role, botGenerationDetails.botDifficulty, );
    ...
    if (botCacheCount > botGenerationDetails.botCountToGenerate) {
      this.logger.debug(`Cache already has sufficient $ {
        cacheKey
      }
      bots: $ {
        botCacheCount
    ...
    const botGenerationPromises = Array.from({
      length: botsToGenerate
    },
    async(_, i) = >{
      try {
        ...
        this.logger.error(`Failed to generate bot#$ {
          i + 1
        }: $ {
          error.message
        }`);
    ...
    this.logger.debug(`Generated $ {
      botGenerationDetails.botCountToGenerate
    }
    $ {
  ...
  /**
     ...
     */
  protected async generateSingleBotAndStoreInCache(botGenerationDetails: IBotGenerationDetails, sessionId: string, cacheKey: string, ) : Promise < void > {
  ...
  public getBotCap(location: string) : number {
    ...
    if (location === "default") {
      this.logger.warning(this.localisationService.getText("bot-no_bot_cap_found_for_location", location.toLowerCase()), );
    }
