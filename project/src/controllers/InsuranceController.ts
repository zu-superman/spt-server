...
@injectable() export class InsuranceController {
    ...
    protected filterInsuredItems(sessionID: string, time ? :number) : IInsurance[] {
      ...
      if (profileInsuranceDetails.length > 0) {
        this.logger.debug(`Found $ {
          profileInsuranceDetails.length
        }
        insurance packages in profile $ {
          sessionID
        }`);
      }
    ...
    protected processInsuredItems(insuranceDetails: IInsurance[], sessionID: string) : void {
      ...
      insurance packages, which includes a total of $ {
    ...
    protected removeInsurancePackageFromProfile(sessionID: string, insPackage: IInsurance) : void {
      ...
      profile.insurance = profile.insurance.filter((insurance) = >insurance.traderId !== insPackage.traderId || insurance.systemData.date !== insPackage.systemData.date || insurance.systemData.time !== insPackage.systemData.time || insurance.systemData.location !== insPackage.systemData.location, );
  
      this.logger.debug(`Removed processed insurance package.Remaining packages: $ {
        profile.insurance.length
      }`);
    }
    ...
    protected findItemsToDelete(rootItemParentID: string, insured: IInsurance) : Set < string > {
      ...
      if (toDelete.size) {
        this.logger.debug(`Marked $ {
          toDelete.size
        }
        items
        for deletion from insurance.`);
      }
    ...
    /**
       ...
       */
    protected populateParentAttachmentsMap(rootItemParentID: string, insured: IInsurance, itemsMap: Map < string, IItem > , ) : Map < string,
    IItem[] > {
      const mainParentToAttachmentsMap = new Map < string,
      IItem[] > ();
      for (const insuredItem of insured.items) {
        ...
        if (!parentItem && insuredItem.parentId !== rootItemParentID) {
          this.logger.warning(this.localisationService.getText("insurance-unable_to_find_parent_of_item", {
    ...
    /**
       ...
       */
    protected removeNonModdableAttachments(parentAttachmentsMap: Map < string, IItem[] > , itemsMap: Map < string, IItem > , ) : Map < string,
    IItem[] > {
    ...
    protected processAttachmentByParent(attachments: IItem[], traderId: string, toDelete: Set < string > ) : void {
      ...
      const attachmentsProbabilityArray = new ProbabilityObjectArray < string,
      number > (this.mathUtil, this.cloner);
      for (const attachmentTpl of Object.keys(weightedAttachmentByPrice)) {
        attachmentsProbabilityArray.push(new ProbabilityObject(attachmentTpl, weightedAttachmentByPrice[attachmentTpl]), );
      }
      ...
      this.logger.debug(`Number of attachments to be deleted: $ {
    ...
    protected logAttachmentsBeingRemoved(attachmentIdsToRemove: string[], attachments: IItem[], attachmentPrices: Record < string, number > , ) : void {
      ...
      for (const attachmentId of attachmentIdsToRemove) {
        ...
        Tpl: $ {
          ...
          attachmentPrices[attachmentId]
        }`, );
        index++;
      }
    }
  
    protected weightAttachmentsByPrice(attachments: IItem[]) : Record < string,
    number > {
    ...
    protected sendMail(sessionID: string, insurance: IInsurance) : void {
      ...
      this.mailSendService.sendLocalisedNpcMessageToPlayer(sessionID, this.traderHelper.getTraderById(insurance.traderId), insurance.messageType, insurance.messageTemplateId, insurance.items, insurance.maxStorageTime, insurance.systemData, );
    }
  
    protected IsMapLabsAndInsuranceDisabled(insurance: IInsurance, labsId = "laboratory") : boolean {
      return (insurance.systemData ? .location ? .toLowerCase() === labsId && !this.databaseService.getLocation(labsId).base.Insurance);
    }
    ...
    protected handleLabsInsurance(traderDialogMessages: Record < string, string[] > , insurance: IInsurance) : void {
      ...
      const responseMesageIds = traderDialogMessages.insuranceFailedLabs ? .length > 0 ? traderDialogMessages.insuranceFailedLabs: traderDialogMessages.insuranceFailed;
    ...
    public insure(pmcData: IPmcData, body: IInsureRequestData, sessionID: string) : IItemEventRouterResponse {
      ...
      for (const key of body.items) {
        itemsToPay.push({
          id: Money.ROUBLES,
          // TODO: update to handle different currencies
          count: this.insuranceService.getRoublePriceToInsureItemWithTrader(pmcData, inventoryItemsHash[key], body.tid, ),
        });
      }
    ...
    public insureSoftInserts(item: IItem, pmcData: IPmcData, body: IInsureRequestData) : void {
      ...
      const softInsertSlots = pmcData.Inventory.items.filter((_item) = >_item.parentId === item._id && softInsertIds.includes(_item.slotId.toLowerCase()), );
  
      for (const softInsertSlot of softInsertSlots) {
        this.logger.debug(`SoftInsertSlots: $ {
          softInsertSlot.slotId
        }`);
        pmcData.InsuredItems.push({
          tid: body.tid,
          itemId: softInsertSlot._id
        });
      }
    }
    ...
    public cost(request: IGetInsuranceCostRequestData, sessionID: string) : IGetInsuranceCostResponseData {
      ...
      const inventoryItemsHash: Record < string,
      IItem > ={};
      ...
      for (const trader of request.traders) {
        ...
        for (const itemId of request.items) {
          ...
          if (!inventoryItemsHash[itemId]) {
            this.logger.debug(`Item with id: $ {
              itemId
            }
            missing from player inventory, skipping`);
            continue;
          }
          items[inventoryItemsHash[itemId]._tpl] = this.insuranceService.getRoublePriceToInsureItemWithTrader(pmcData, inventoryItemsHash[itemId], trader, );
        }
